...
export default class JobCell extends PureComponent<ICell> {
	...
	renderTag() {
		...
		// isStop ? 0 :
		const {
			cellItem: { isUrgent, isPartimeJob, isHotjob, isStop },
		} = this.props
		const basicView = [
			isPartimeJob
		...
		return <View style={styles.tagView}>{basicView}</View>
	}
			...
			<NextPressable
				...
				<View style={styles.cellTitleView}>
					<View style={styles.cellTitleTagView}>
						<Text style={styles.cellTitle}>{cellItem.name}</Text>
						{this.renderTag()}
					</View>
					{cellItem.isStop ? (
						<Text style={styles.cellStop}>停止招聘</Text>
					) : (
				...
				<View style={styles.cellCompanyView}>
					<Text style={styles.cellCompany}>{cellItem.company}</Text>
					<Text style={styles.cellCompany}>{cellItem.financing}</Text>
					<Text style={styles.cellCompany}>{cellItem.staffAmount}</Text>
				</View>
				<View
					style={[
						styles.cellJobView,
						showCellTime && { borderBottomWidth: 0, paddingBottom: 0 },
					]}>
					<Text style={styles.cellExperience}>{cellItem.experience}</Text>
					<Text style={styles.cellExperience}>{cellItem.education}</Text>
					<Text style={styles.cellExperience}>{cellItem.location}</Text>
				</View>
				<View style={styles.interviewerView}>
					<View style={styles.interviewerIcon} />
					<Text style={styles.cellInterviewer}>{cellItem.interviewer}</Text>
					{showNewTag && (
