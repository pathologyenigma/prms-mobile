...
export default class extends Component {
  ...
  toggleSwitchToValue = (result, toValue, callback = () => null) => {
    ...
    if (result) {
      this.animateSwitch(toValue, () => {
      	this.state.value = toValue
      	this.props.onValueChange && this.props.onValueChange(toValue)
      	callback()
        // this.setState({
        //   value: toValue,
  ...
  render() {
    ...
    const interpolatedBackgroundColor = Animated.interpolateColors(switchAnimation, {
      inputRange: [-this.offset, this.offset],
      outputColorRange: [backgroundInactive, backgroundActive],
      extrapolate: 'clamp'
    })

    const interpolatedCircleColor = Animated.interpolateColors(switchAnimation, {
      inputRange: [-this.offset, this.offset],
      outputColorRange: [circleColorInactive, circleColorActive],
      extrapolate: 'clamp'
    })
    ...
    const interpolatedTranslateX = Animated.add(Animated.divide(switchAnimation, 2), Animated.multiply(Animated.sub(handlerAnimation, this.handlerSize - 2), Animated.divide(switchAnimation, -this.offset * 2)))
